...
class Foothold {
  ...
  static loadFootholds(current, id?: number): Foothold[] {
    var list = [];
    if (current.x1) {
  ...
  static linkFootholds(footholds: Foothold[]) {
    var lookup = {};
    footholds.forEach(f => lookup[f.id] = f);
    footholds.forEach(f => f.next = lookup[f.nextId]);
    footholds.forEach(f => f.prev = lookup[f.prevId]);
  ...
  draw(ctx: CanvasRenderingContext2D, isPlayerTouching: boolean) {
    ctx.beginPath();
    ctx.lineWidth = 1;
  ...
  getIntersection(pointX: number, pointY: number, nextPosX: number, nextPosY: number): number {
    return Foothold.getLineYIntersection(
      pointX, pointY,
      nextPosX, nextPosY,
      this.line.x1,
      this.line.y1,
